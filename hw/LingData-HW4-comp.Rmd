...
```{r}
data = pivot_wider(mono_socio, names_from = "QuestionCategory", values_from = "Responce")
```
...
```{r}
length(data$sex[data$sex == "male"])
length(data$sex[data$sex == "female"])
```
...
```{r}
length(data$education[data$education == "College"])
length(data$education[data$education == "UniversityDegree"])
length(data$education[data$education == "Postgraduate"])
length(data$education[data$education == "PhDongoing"])
```
...
length(data$handedness[data$handedness == "left"])
```
...
RT is not following normal distribution because of long right tail that consists of some outliers.
...
We still cant say that RTlog approximately follows normal distribution because of the skewness and some outliers at the right part.
...
```{r}
RTabove600 <- mono[mono$RT > 599,]
NoFillers <- RTabove600[(RTabove600$TypeOfStimuli != "FillerUnacceptable") & (RTabove600$TypeOfStimuli != "FillerAcceptable"),]
RTlog1 = log10(NoFillers$RT)
mut1 = NoFillers %>% mutate(RT = RTlog1)
mono1 <- mut1[(mut1$RT < (mean(RTlog1) + 3*sd(RTlog1))) & (mut1$RT > (mean(RTlog1) - 3*sd(RTlog1))),]
mono1

mono1$ParticipantID <- as.factor(mono1$ParticipantID)
mono1$TypeOfStimuli <- as.factor(mono1$TypeOfStimuli)
mono1$WordOrder <- as.factor(mono1$WordOrder)
mono1$AcceptabilityJ <- as.factor(mono1$ResponseAcceptabilityJudgement)
...
#3select(ParticipantID, TypeOfStimuli, WordOrder, AcceptabilityJ = 
...
mpg_plot <- ggplot(mono1, aes(x = TypeOfStimuli, y = RT, fill = AcceptabilityJ)) +
...
```{r}
ggplot(mono1, aes(x = WordOrder, y = RT, fill = AcceptabilityJ)) +
  geom_col(colour = "black", position = "dodge")
```
...
```{r}
mono1 %>%
  ggplot( aes(x=AcceptabilityJ, y=RT, fill=WordOrder)) +
...
```{r}
ggline(mono1, "AcceptabilityJ", "RT", add = "mean_se")
...
select(ParticipantID, WordOrder, RT) %>%
group_by(ParticipantID, WordOrder) %>%
summarise(RTlog = mean(RT))
...
They are not significantly different. Probably if we had more observations the difference between them would be even smaller.
...
t.test(mean_rtlog$diff, mu=0, alternative="less")
```
...
Reaction time for Incongruent stimuli is greater then for Congruent stimuli.

How can you interpret difference with the result of 2.3?

In 2.3 we didnt use `paired = TRUE`, so we assumed that Congruent and Incongruent stimuli are independant, whilst in 2.4 we assumed that they are dependant.
...
```{r}
t.test(mean_rtlog$Congruent, mean_rtlog$Incongruent, paired = TRUE, alternative="less")
...
```{r}
incong_rtlog <- mono1 %>%
select(ParticipantID, WordOrder, TypeOfStimuli, RT) %>%
group_by(ParticipantID, TypeOfStimuli, WordOrder) %>%
summarise(RTlog = mean(RT))

incong_rtlog <- incong_rtlog[incong_rtlog$WordOrder == "Incongruent", c("ParticipantID", "TypeOfStimuli", "RTlog")]
...
```{r}
fm <- aov(incong_rtlog$RTlog ~ incong_rtlog$TypeOfStimuli)
summary(fm)
```
p-value is less then 0.05 so we can reject the null hypothesis (no statistically significant difference between types of stimuli). So there is a statistically significant difference between them.
